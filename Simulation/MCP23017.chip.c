...
bool on_i2c_write(void *user_data, uint8_t data) {
  ...
  // BUG FIX: Do NOT wipe inputMask to 0 here. 
  // Update it bit-by-bit below so it doesn't miss rapid responses from the 74LS00.
  ...
  for (uint8_t i=0; i<8; i++) {
    ...
    if (data & (1<<i)) {
      // Configured as INPUT / HIGH
      chip->inputMask |= (1 << bitIdx); // Safely set this bit to 1
      pin_mode(targetPin, INPUT_PULLUP);
      pin_watch(targetPin, &(chip->io_watch_config));
    } else {
      // Configured as OUTPUT / LOW
      chip->inputMask &= ~(1 << bitIdx); // Safely clear this bit to 0
